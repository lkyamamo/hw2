...
#ifndef kwsys_FStream_hxx
#define kwsys_FStream_hxx

#include <kwsys/Configure.hxx>

#include <kwsys/Encoding.hxx>

#include <fstream>
#if defined(_WIN32)
#  if !defined(_MSC_VER) && kwsys_CXX_HAS_EXT_STDIO_FILEBUF_H
...
namespace kwsys {
#if defined(_WIN32) &&                                                        \
  (defined(_MSC_VER) || kwsys_CXX_HAS_EXT_STDIO_FILEBUF_H)
#  if defined(_NOEXCEPT)
#    define kwsys_FStream_NOEXCEPT _NOEXCEPT
#  else
#    define kwsys_FStream_NOEXCEPT
#  endif
...
public:
  ...
  ~basic_ifstream() kwsys_FStream_NOEXCEPT { close(); }
};
...
public:
  ...
  ~basic_ofstream() kwsys_FStream_NOEXCEPT { close(); }
};

typedef basic_ifstream<char> ifstream;
typedef basic_ofstream<char> ofstream;

#  undef kwsys_FStream_NOEXCEPT
#else
using std::ofstream;
using std::ifstream;
#endif
...
// seekable stream).
kwsys_EXPORT BOM ReadBOM(std::istream& in);
}
}
